...
function getwellMergeStores(local, remote){
  const localPatients  = Array.isArray(local && local.patients)  ? local.patients  : [];
  const remotePatients = Array.isArray(remote && remote.patients) ? remote.patients : [];
  ...
  localPatients.forEach(patient => {
    if(patient && patient.id) byId.set(String(patient.id), patient);
  });

  remotePatients.forEach(patient => {
    ...
    const mineTime   = getwellRecordTime(mine);
    const theirsTime = getwellRecordTime(patient);

    if(theirsTime > mineTime){
      byId.set(id, patient);
      remoteWon = true;
    }else if(mineTime > theirsTime){
      localWon = true;
    }else if(JSON.stringify(mine) !== JSON.stringify(patient)){
      /*
        Same timestamp but different content (typically a
        record that predates stamping). Prefer the Sheet,
        which is the shared source of truth.
      */
      byId.set(id, patient);
      remoteWon = true;
    }
  ...
  /* A patient present locally but absent from the Sheet still needs pushing up. */
  const remoteIds = new Set(remotePatients.filter(p => p && p.id).map(p => String(p.id)));
  localPatients.forEach(patient => {
    if(patient && patient.id && !remoteIds.has(String(patient.id))) localWon = true;
  });

  return {
    merged: {...(remote || {}), ...(local || {}), patients: Array.from(byId.values())},
    remoteWon,
    localWon
  };
}
...
function getwellSyncRemoteStore(allowReload){
  ...
  getwellRemoteRead(payload => {
    ...
    if(!payload || payload.ok !== true || !payload.data){
      /* Offline or misconfigured: keep working from the local cache. */
      return;
    }
    ...
    const result = getwellMergeStores(local, remote);
    ...
    if(result.localWon){
      /* Local holds records the Sheet does not have, or newer ones. Push them up. */
...
function saveStore(
  ...
  /*
    ...
    The result is reported so a failed write is never
    mistaken for a successful one.
  */
  return getwellRemoteSave(
    snapshot
  ).then(result => {
    if(!result.ok){
      getwellNotify(
        "Saved on this device, but NOT to Google Sheets. " + result.error,
        "error"
      );
    }
    return result;
