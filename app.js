      ...
      visits: [
        {
          ...
          notes: "",
          billing: {
            injection: { price: 120, notes: "Tirzepatide 2.5 mg" },
            medication: { price: 0, notes: "" },
            treatment: { price: 50, notes: "Gut Detox" },
            other: { price: 0, notes: "" },
            panel: 120,
            selfPay: 50,
            panelClaimAmount: 0,
      ...
      visits: [
        {
          ...
          notes: "",
          billing: {
            injection: { price: 100, notes: "" },
            medication: { price: 20, notes: "" },
      ...
      visits: [
        {
          ...
          notes: "",
          billing: {
            injection: { price: 150, notes: "Semaglutide 1 mg" },
            medication: { price: 0, notes: "" },
            treatment: { price: 80, notes: "Physio" },
            other: { price: 0, notes: "" },
            panel: 200,
            selfPay: 30,
...
function getPatient(id) {
  return store().patients.find((patient) => patient.id === id) || null;
}
...
function getPanelName(patient) {
  ...
  if (patient.panelProvider === "SELF_PAY") {
    return "Self-Pay";
  }

  if (patient.panelProvider === "Other") {
    return patient.otherPanelName || "Panel";
  }
...
function panelEligible(visit) {
  return Math.max(0, +ensureVisit(visit).billing.panel || 0);
}
...
function finance(patient) {
  ...
  (patient.visits || []).forEach((visit) => {
    ...
    result.grand += visitTotal(visit);
    result.claimable += panelEligible(visit);
    result.claimed += totalClaimed(visit);

    result.injection += +billing.injection.price || 0;
    result.medication += +billing.medication.price || 0;
    result.treatment += +billing.treatment.price || 0;
    result.selfpay += +billing.selfPay || 0;
  });

  result.balance = Math.max(
    0,
...
function daysSince(dateKey) {
  ...
  const visitDate = new Date(dateKey + "T00:00:00");
  const today = new Date();

  today.setHours(0, 0, 0, 0);

  return Math.floor(
    (today - visitDate) / 86400000
  );
}
...
function alerts() {
  return store().patients
    .map((patient) => {
      const visit = latestVisit(patient);
      const days = daysSince(visit?.dateKey);
      ...
      return {
        ...
        name: patient.name,
        days,
        level: days >= 7 ? "overdue" : "warning",
        text: `${days} days since last recorded visit.`
      };
    })
    .filter(Boolean)
    .sort((a, b) => {
    ...
    ? notifications
        .map(
          (item) => `
            <div
              class="notif-item"
              onclick="location.href='patient-profile.html?patient=${encodeURIComponent(
                item.id
              )}'"
            >
              <span class="notif-dot ${item.level}"></span>
...
document.addEventListener("click", (event) => {
  const wrapper =
    document.getElementById("notifWrap");
...
function header(title, subtitle, active) {
  return `
    <header class="topbar">
          ...
          >
            ...
            <div class="notif-head">
              ...
              <button
                ...
                onclick="document.getElementById('notifPanel').hidden=true"
              >
                ×
              </button>
