...
function ensureClaims(p){if(!Array.isArray(p.claims))p.claims=[];return p.claims}
...
function claimsTotal(p){return ensureClaims(p).reduce((s,c)=>s+(+c.amount||0),0)}
function finance(p){
  ...
  return {grand,claimed,balance:Math.max(0,grand-claimed),injection,medication,treatment,selfpay}
}
function latestVisit(p){
  return [...(p.visits||[])].sort((a,b)=>(a.dateKey||"").localeCompare(b.dateKey||"")).at(-1)||null
}
function daysSince(d){
  if(!d)return null;
  const a=new Date(d+"T00:00:00"), b=new Date(); b.setHours(0,0,0,0);
  return Math.floor((b-a)/86400000)
}
function alerts(){
  return (store().patients||[]).map(p=>{
    const d=daysSince(latestVisit(p)?.dateKey);
    if(d===null||d<5)return null;
    return {id:p.id,name:p.name,days:d,level:d>=7?"overdue":"warning"}
...
function applyTheme(t){
  ...
  if(b){b.textContent=t==="dark"?"☀":"☾";b.title=t==="dark"?"Switch to Day Mode":"Switch to Night Mode"}
}
...
function toggleNotifications(e){
 e.stopPropagation(); const p=document.getElementById("notifPanel"); if(p){p.hidden=!p.hidden;if(!p.hidden)renderNotifications()}
}
function renderNotifications(){
 const a=alerts(), c=document.getElementById("notifCount"), b=document.getElementById("notifBody");
 if(!c||!b)return;
 c.hidden=!a.length;c.textContent=a.length>99?"99+":a.length;
 b.innerHTML=a.length?a.map(x=>`<div class="notif-item" onclick="location.href='patient-profile.html?patient=${encodeURIComponent(x.id)}'"><span class="notif-dot ${x.level}"></span><div><div class="notif-name">${x.name}</div><div class="notif-text">${x.level==="overdue"?"Overdue":"Due for Follow-Up"} · ${x.days} days since last visit.</div></div></div>`).join(""):`<div class="notif-empty">No patients are due.</div>`
}
document.addEventListener("click",e=>{
 const w=document.getElementById("notifWrap"),p=document.getElementById("notifPanel");
 if(w&&p&&!w.contains(e.target))p.hidden=true
});
document.addEventListener("DOMContentLoaded",()=>{initTheme();renderNotifications()});
